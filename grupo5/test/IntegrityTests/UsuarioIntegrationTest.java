...
public class UsuarioIntegrationTest {
    ...
    public void testAddUsuario() {
        running(testServer(3333, fakeApplication(inMemoryDatabase("test", ImmutableMap.of("MODE", "MYSQL")))), HTMLUNIT, browser -> {
            ...
            Http.RequestBuilder x = fakeRequest(GET, "/usuarios");
            Result t = route(x);
            ...
            // Creo una entidad
            Http.RequestBuilder x1 = fakeRequest(POST, "/usuarios");
            ...
            Http.RequestBuilder x2 = fakeRequest(GET, "/usuarios");
            ...
            Http.RequestBuilder x3 = fakeRequest(POST, "/usuarios");
            Result t3 = route(x3);
    ...
    public void testRemoveUsuario() {
        running(testServer(3333, fakeApplication(inMemoryDatabase("test", ImmutableMap.of("MODE", "MYSQL")))), HTMLUNIT, browser -> {
            ...
            // Elimino una entidad
            Http.RequestBuilder x2 = fakeRequest(DELETE, "/usuarios/"+jsonAns.get("_id").asText());
            Result t2 = route(x2).as("application/json");
            assertTrue(t2.status() == 200);
            assertTrue(t2.contentType().toString().equals("Optional[application/json]"));

            // Busco la entidad borrada
            Http.RequestBuilder x3 = fakeRequest(GET, "/usuarios/"+jsonAns.get("_id").asText());
    ...
    public void testGetByIdUsuario() {
        running(testServer(3333, fakeApplication(inMemoryDatabase("test", ImmutableMap.of("MODE", "MYSQL")))), HTMLUNIT, browser -> {
            ...
            Http.RequestBuilder x2 = fakeRequest(GET, "/usuarios/"+jsonAns.get("_id").asText());
            Result t2 = route(x2).as("application/json");
            assertTrue(t2.status() == 200);
            assertTrue(t2.contentType().toString().equals("Optional[application/json]"));
