...
public class MinaSubterraneaIntegrationTest {
    ...
    public void testAddRegistroMina() {
        running(testServer(3333, fakeApplication(inMemoryDatabase("test", ImmutableMap.of("MODE", "MYSQL")))), HTMLUNIT, browser -> {
            ...
            // Creo una entidad
            Http.RequestBuilder x1 = fakeRequest(POST, "/minas");
            ...
            Http.RequestBuilder x2 = fakeRequest(GET, "/minas");
            ...
            Http.RequestBuilder x3 = fakeRequest(POST, "/minas");
            Result t3 = route(x3);
    ...
    public void testRemoveMina() {
        running(testServer(3333, fakeApplication(inMemoryDatabase("test", ImmutableMap.of("MODE", "MYSQL")))), HTMLUNIT, browser -> {
            ...
            // Elimino una entidad
            Http.RequestBuilder x2 = fakeRequest(DELETE, "/minas/"+jsonAns.get("_id").asText());
            Result t2 = route(x2).as("application/json");
            assertTrue(t2.status() == 200);
            assertTrue(t2.contentType().toString().equals("Optional[application/json]"));

            // Busco la entidad borrada
            Http.RequestBuilder x3 = fakeRequest(GET, "/minas/"+jsonAns.get("_id").asText());
    ...
    public void testGetByIdMina() {
        running(testServer(3333, fakeApplication(inMemoryDatabase("test", ImmutableMap.of("MODE", "MYSQL")))), HTMLUNIT, browser -> {
            ...
            Http.RequestBuilder x2 = fakeRequest(GET, "/minas/"+jsonAns.get("_id").asText());
            Result t2 = route(x2).as("application/json");
            assertTrue(t2.status() == 200);
            assertTrue(t2.contentType().toString().equals("Optional[application/json]"));
